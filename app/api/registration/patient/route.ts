...
export async function POST(request: Request) {
  try {
    const { name, email, password, phone, age } = await request.json()
    ...
    try {
      ...
      console.error('Patient Registration Database query failed:', dbError)
      return new Response(
    ...
    // 4. Insert new patient into database
    try {
      const result: any = await query(
        'INSERT INTO patients (name, email, password_hash, phone, age) VALUES (?, ?, ?, ?, ?)',
        [name, email, passwordHash, phone, age]
      )

      console.log('Patient registered successfully:', result.insertId)
      ...
      console.error('Patient Registration Database insert failed:', insertError)
      return new Response(
